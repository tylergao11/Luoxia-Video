...
```bash
# 建议先把样例拷出去改；默认输出根目录就是仓库的 output/
python -m src.luoxia run contracts/examples/novel.sample.txt --work-id demo --budget 5
...
```
output/projects/<work_id>/beats.json
output/projects/<work_id>/characters/*.png     # 每角色一张定妆图，全片锁脸的底图
output/episodes/<episode_id>/timeline.json
output/episodes/<episode_id>/audio/*.wav
output/episodes/<episode_id>/stills/*.png
output/episodes/<episode_id>/video/*.mp4
output/episodes/<episode_id>/final.mp4
```
...
python -m src.luoxia beats-select output/projects/demo/beats.json --max-repair-severity medium
python -m src.luoxia sheets output/projects/demo/beats.json
python -m src.luoxia beats-bridge output/projects/demo/beats.json ep01
...
2. **`output/projects/<work_id>/characters/` 里的定妆图。** 这是全片锁脸的源头，它不对后面每一张都不对。改 `cast[].appearance` 会让缓存失效并重出；不动就一直复用，成本只付一次。

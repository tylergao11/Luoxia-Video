...
import { Palette, Layout, Film, BookOpen, Users, Video, Settings, Key, MessageSquareCode, Clapperboard, Scissors } from "lucide-react";
...
import BeatsReview from "@/components/modules/BeatsReview";
...
// Luoxia-Video pipeline: novel/script → beats (content authority) → style/cast → board → assembly.
// Legacy path keeps assets/motion split for older projects.
const LEGACY_STEPS = [
    { id: "script", label: "1. Script", icon: BookOpen },
    { id: "beats", label: "2. Beats", icon: Scissors },
    { id: "art_direction", label: "3. Art Direction", icon: Palette },
    { id: "assets", label: "4. Assets", icon: Users },
    { id: "storyboard", label: "5. Storyboard", icon: Layout },
    { id: "motion", label: "6. Motion", icon: Video },
...
const UNIFIED_STEPS = [
    ...
    { id: "beats", label: "2. Beats", icon: Scissors },
    { id: "art_direction", label: "3. Art Direction", icon: Palette },
    { id: "cast", label: "4. Cast", icon: Users },
    { id: "storyboard_r2v", label: "5. Storyboard", icon: Clapperboard },
...
export default function ProjectClient({ id, breadcrumbSegments }: { id: string; breadcrumbSegments?: BreadcrumbSegment[] }) {
    ...
    const steps = useMemo(() => {
        ...
        const statusFor = (id: string): { status?: "ready" | "idle" | "gated"; statusLabel?: string } => {
            switch (id) {
                case "beats":
                    // Honest: no beats field on Script yet — idle until user opens step.
                    return { status: "idle" };
                case "art_direction":
    ...
    const segments = breadcrumbSegments || [{ label: "Luoxia-Video", hash: "#/" }, { label: currentProject.title }];
